...
def ReactionetLasso(options):
	...
	for cv in range(1, Ncv+1):
		...
		if  not CheckFileExist(FileName, StatusFile):
			Param = '"(\'{0}\', {1:d}, \'{2}\', {3:d}, {4:d}, {5:f}, \'{6}\')"'.format(options.ModelName, cv,
				options.GradientType, options.NMom, options.Nboot, options.p, options.PriorGraph)
	...
	if  not CheckFileExist(FileName, StatusFile):
		Param = '"(\'{0}\', \'{1}\', {2:d}, {3:d}, {4:f}, \'{5}\')"'.format(options.ModelName,
				options.GradientType, options.NMom, options.Nboot, options.p, options.PriorGraph)
