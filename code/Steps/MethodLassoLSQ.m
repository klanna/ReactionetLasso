function [StatLassoLL, RunTimeS, RunTimeSname] = MethodLassoLSQ( FolderNames, indx_I, indx_J, values, N_obs, N_re, b, constr, kTrue, Av, bv)
    ts = tic;
    RunTimeSname = 'MethodLassoLSQ';
    fprintf('----------------%s----------------\n', RunTimeSname);
    if FolderNames.CV
    ...
    %%  weight on error distribution
        ...
        lambda_list = sort(10.^[-15:2], 'descend');
        
        for i = 1:length(lambda_list)
            lambda = lambda_list(i);
            StatLassoLL(i).lambda = lambda;
            
            Xi = LassoADMMlsqr2(Aw, b, weights, lambda, constrW);
