function FdrCVandFreq( A, titlename, LegendNames, FileName)
% A = [Nlines, Nsubplots];
% titlename = {Nsubplots}
    ...
    for i = 1:N
        ...
        for j = 1:Nlines
            ...
            M_tp = M(:, 1);
            if i ~= 2
                plot(M_fp, M_tp, LineSymb{i}, 'color', MyColor(i, :), 'MarkerSize', MarkerSize, 'LineWidth', lwidth*i)
            else
                plot(M_fp, M_tp, LineSymb{i}, 'color', MyColor(1, :), 'MarkerSize', MarkerSize, 'LineWidth', lwidth*2)
            end
            hold on
            m1x(j) = min(M_fp);
            m1y(j) = min(M_tp);
